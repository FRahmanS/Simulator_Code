% join module
%  % all same rs has changed before
% 
%  tic
% clear all
% close all
...
test=0; %T3 only with 0.15 offspring size
%%% set parameter----------------------------------------------------------
NNP=100;
NP=100;                                  %number of population.
MP=Male_R;                         %number of male.
...
if(LengthNewG1>0 && LengthNewG1<=MP)
...
MsizeLeft=size(LeftG.MOSize,1);


if((MP-LengthNewG1)>0 && MsizeLeft>0)
MP1=MP-LengthNewG1;
if(MsizeLeft<=MP1)

init_robot.r(LengthNewG1+1:LengthNewG1+MsizeLeft)=LeftG.MOSize;
%init_robot.r(LengthNewG1+1+MP:LengthNewG1+sizeLeft+MP)=LeftG.FOSize;

init_robot.energy(LengthNewG1+1:LengthNewG1+MsizeLeft)=LeftG.MOEng;        
%init_robot.energy(LengthNewG1+1+MP:LengthNewG1+sizeLeft+MP)=LeftG.FOEng;

init_robot.Robot_gene((LengthNewG1+1:LengthNewG1+MsizeLeft),:)=LeftG.MaleOf;        
...
elseif(LengthNewG1>0 && LengthNewG1>MP)
...
if(LengthNewG2>0 && LengthNewG2<=FP)
...
FsizeLeft=size(LeftG.FOSize,1);


if((FP-LengthNewG2)>0 && FsizeLeft>0)
FP1=FP-LengthNewG2;
if(FsizeLeft<=FP1)

%init_robot.r(LengthNewG1+1:LengthNewG1+sizeLeft)=LeftG.MOSize;
init_robot.r(LengthNewG2+1+MP:LengthNewG2+FsizeLeft+MP)=LeftG.FOSize;

%init_robot.energy(LengthNewG1+1:LengthNewG1+sizeLeft)=LeftG.MOEng;        
init_robot.energy(LengthNewG2+1+MP:LengthNewG2+FsizeLeft+MP)=LeftG.FOEng;

%init_robot.Robot_gene((LengthNewG1+1:LengthNewG1+sizeLeft),:)=LeftG.MaleOf;        
init_robot.Robot_gene((LengthNewG2+1+MP:LengthNewG2+FsizeLeft+MP),:)=LeftG.FemaleOf;
%LeftG.MaleOf=[];
LeftG.FemaleOf=[];
%LeftG.MOEng=[];
LeftG.FOEng=[];
...
randF=randperm(FsizeLeft,FP1);   
%init_robot.r(LengthNewG1+1:MP)=LeftG.MOSize(randM);
init_robot.r(LengthNewG2+1+MP:NP)=LeftG.FOSize(randF);

%init_robot.energy(LengthNewG1+1:MP)=LeftG.MOEng(randM);        
init_robot.energy(LengthNewG2+1+MP:NP)=LeftG.FOEng(randF);

%init_robot.Robot_gene((LengthNewG1+1:MP),:)=LeftG.MaleOf(randM,:);        
init_robot.Robot_gene((LengthNewG2+1+MP:NP),:)=LeftG.FemaleOf(randF,:);
...
elseif(LengthNewG2>0 && LengthNewG2>FP)
...
%             grbm=find(male_offspring(:,45)>0.0009);
%             if (~isempty(grbm))
%             male_offspring(grbm,45)=0.00009;    
%             end
%             grbf=find(female_offspring(:,45)>0.00009);
%             if (~isempty(grbf))
%             female_offspring(grbf,45)=0.00009;
%             end
            ...
            calengOf=(ef(rpt)'*0.2);
%             calengOf=(ef(rpt)'*0.4)+(sm(rpt)'*rs.robot.growth_cost*mate_end_time);
...
% figure(1)
% plot(analyze.TotMate)
% hold on;
% 
% figure(2)
% k= find(AnTable.Genetation==gen);
% hist(AnTable.Male_parent(k,50)*300);
% 
% figure(3)
% k= find(AnTable.Genetation==gen);
% hist(AnTable.Female_parent(k,50)*300);
% 
% 
% if(analyze.TotMate(gen,1)==0)
% 
%     zerogen=rs.robot
%     
% %   pause  
...
Ag4=AG(301:400,1:100);
Ag5=AG(401:500,1:100);
Ag6=AG(501:600,1:100);
Ag7=AG(601:700,1:100);
clear AG;
lpn=int2str(loopx);
nb=int2str(MP);
name=strcat('cr8f100/',lpn,'NFTest',nb);

save (name);
% TO=struct2table(AnTable);
% TG=struct2table(analyze);
% writetable(TO,'OffspringInfo.txt');
% 
% writetable(TG,'GenerationInfoT3.txt');
% save('testT3')

% if height(T)>0
% chrgen=int2str(gen);
% chrgen=strcat(chrgen,'.txt');
% filename=strcat('generation',chrgen);
% writetable(T, filename);
% end
toc
% for llops end for evolutiona    
